...
- 2026-04-08: Re-hardened `scripts/dead-letter.sh` to prefer the absolute `/opt/homebrew/bin/openclaw` path, fail fast when the CLI is missing, resolve Telegram bot tokens from env-backed config, normalize timestamps to SGT in alerts, and verified the wrapper with a simulated failing command (exit 23). Ran `openclaw doctor --fix` and `openclaw doctor`; system health remains operational with only existing non-blocking warnings.
