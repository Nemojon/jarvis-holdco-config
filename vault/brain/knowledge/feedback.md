...
| 2026-04-10 | Cron / nightly brain save | The nightly brain save failed again before useful work: first `gpt-5.4-pro` was invoked with unsupported `thinking=low`, then a retry hit an LLM idle timeout before fallback to Codex. | Escalate from recurring annoyance to hard reliability defect; cron/runtime defaults need to be made model-compatible and resilient before the next scheduled run. |
